...
const deleteMeetingsByVendor = async (req, res) => {
  try {
    ...
    const deletedMeetings = await meetingModel.deleteMany({ vendorId: id });
    ...
    res.status(500).json({ message: "Error deleting meetings", error: error.stack });
  }
};

module.exports = { create, getMeetingsByVendor,deleteMeetingsByVendor };
