...
const deleteVendorHelpSupportRequest = async (req, res) => {
  try {
    const { vendorid } = req.params;
    if (!vendorid) {
      return res.status(400).json({ message: "Vendor ID is required." });
    }
    const deletedRequest = await VendorHelpSupport.findByIdAndDelete(vendorid);
    if (!deletedRequest) {
      return res.status(404).json({ message: "Help support request not found." });
    }
    return res.status(200).json({
      message: "Vendor help and support request deleted successfully.",
      deletedRequest,
    });
  } catch (error) {
    console.error("Error deleting vendor help and support request:", error);
    return res.status(500).json({
      message: "Server error while deleting the vendor help and support request.",
      error: error.message,
    });
  }
};
...
module.exports = {
  ...
  deleteVendorHelpSupportRequest,
};
